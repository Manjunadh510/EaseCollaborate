...
const createTask = async (req, res) => {
  try {
    ...
    // Check if user is project member and has manager role
...
const updateTaskTitle = async (req, res) => {
  try {
    ...
    // Check permissions: manager can edit any, contributor only own assigned tasks
    if (projectMember.role === "contributor") {
      ...
      if (!isAssigned) {
        return res.status(403).json({
          message: "Contributors can only edit tasks they are assigned to",
        });
      }
    } else if (projectMember.role !== "manager") {
      return res.status(403).json({
        message: "Only managers and assigned contributors can edit tasks",
      });
    }
...
const updateTaskAssignees = async (req, res) => {
  try {
    ...
    // Check permissions: only managers can assign tasks
    const projectMember = project.members.find(
      (member) => member.user.toString() === req.user._id.toString()
    );
...
const updateTaskPriority = async (req, res) => {
  try {
    ...
    // Check permissions: only managers can change priority
    if (projectMember.role !== "manager") {
      return res.status(403).json({
        message: "Only managers can change task priority",
      });
    }
...
const deleteTask = async (req, res) => {
  try {
    ...
    // Check if user is a project member and has manager role
    const projectMember = project.members.find(
      (member) => member.user.toString() === req.user._id.toString()
    );
    ...
    // Delete all activity logs associated with the task
    await ActivityLog.deleteMany({ resource_id: taskId });

    // Remove task from project
    project.tasks = project.tasks.filter(
      (proj_task) => proj_task.toString() !== taskId
    );
    await project.save();
